...
#: 탐지 대상 개인정보 라벨 (10종). **닫힌 집합이고, 여기 없는 것은 탐지하지 않는다.**
#:
#: 이 튜플이 프롬프트의 라벨 목록과 guided-decoding enum 을 **동시에** 만든다.
#: 그래서 여기서 뺀 라벨은 모델이 낼 수 없다 — 프롬프트가 실수로 언급해도
#: 스키마가 막는다. 반대로 여기에 넣기만 하고 프롬프트 형식표에 자리수를
#: 적어 주지 않으면 모델이 그 라벨을 언제 써야 하는지 모른다
#: (``tests/test_schema.py`` 가 그 불일치를 잡는다).
#:
#: 한때 18종이었다. 위 10종에 ``FOREIGN_ID`` ``DRIVER_LICENSE`` ``BIZ_NO``
#: ``CORP_NO`` ``ORG`` ``TITLE`` ``SIGNATURE`` ``OTHER`` 가 더 있었고, 검출
#: 범위가 이 10종으로 확정되어 나머지를 뺐다. 같이 걷힌 것들:
#: ``verify.py`` 의 ``DIGIT_LENGTHS`` 항목, ``rules/checksums.py`` 의 사업자·
#: 법인·외국인 검증기, ``TEXTLESS_LABELS`` (``SIGNATURE`` 전용이었다).
#:
#: **``OTHER`` 를 뺀 것이 프롬프트에서 가장 큰 차이를 만든다.** 도피 라벨이
#: 없으므로 "종류를 모르겠으면 OTHER" 가 불가능해지고, 지시가 "10종이 아니면
#: 아예 보고하지 마라" 로 바뀐다. 그 지시는 **범위 밖을 버리라는 뜻일 뿐이고,
#: 범위 안에서 종류가 애매한 것까지 버리라는 뜻이 아니다.** 둘을 붙여 놓으면
#: 모델이 애매한 주민등록번호를 조용히 흘린다 — ``llm/prompts.py`` 가 이 둘을
#: 명시적으로 갈라 놓았다.
...
class VlmFinding:
    ...
    Attributes:
        text: 문서에 **적힌 그대로**의 값. 회피 표기라면 회피 표기가 담긴다
            ...
            10종은 모두 읽을 글자가 있는 값이므로 비어 있으면 정상이 아니다 —
            VLM 이 위치는 짚었으나 값을 못 읽은 경우이고, ``locate.py`` 가
            그 건을 ``needs_review`` 로 넘긴다 (버리지는 않는다).
