...
변형마다 규칙을 덧붙이는 방식은 조합 폭발로 유지가 안 된다. 대신 **비교 전에
정규형으로 접는다.** 이 모듈이 쓰이는 곳은 두 군데다.

* ``locate.py`` — 크롭 안에서 VLM 이 읽은 값과 OCR 이 읽은 값을 맞출 때.
  VLM 이 ``"010-1234-5678"``, OCR 이 ``"공1공-1234-5678"`` 로 읽어도 같은 값으로
  매칭되어야 한다. 접지 않으면 회피 표기가 적힌 칸은 전부 ``vlm_coarse`` 로
  떨어진다.
* ``detect.py`` — 타일 겹침 구간에서 같은 값이 두 번 보고된 것을 합칠 때.
