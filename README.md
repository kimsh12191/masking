...
| ③ | 유사 매칭 — `NAME`/`ADDRESS`/`ORG`/`TITLE` **만** | 정확 | `similar` | 있음 |
| ④ | **VLM bbox 와 위치가 겹치는 박스** (텍스트 안 봄) | 정확 | `none` | 없음 |
| ⑤ | 크롭에 OCR 박스가 없음 | 근사 | `none` | 없음 |

**④가 이 설계의 핵심이다.** VLM 이 잘하는 일은 "여기에 이 종류의 개인정보가
...
④는 `FAILED` 박스도 후보에 넣는다. det 는 성공하고 rec 만 실패한 박스는
...
| 박스는 있는데 겹치지 않음 | `vlm_coarse` | `true` | VLM **좌표** 문제 |
...
③ 좌표 확정  크롭 패딩 35% (최소 12px)  업샘플 x2.0  유사매칭 0.65  기하 fallback=ON
