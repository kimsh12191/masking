...
{"text":"901112-2846261","type":"RRN","field":"가족사항 자녀 주민등록번호",
 "bbox_norm":[0.42,0.31,0.68,0.34],"conf":0.9}
```

- `text` — 문서에 **적힌 그대로**. 회피 표기면 회피 표기가 담긴다 (감사 추적).
  단 `type` 은 **원래 값 기준**으로 정한다 (`"구1공8공4"` → `PHONE`).
- `bbox_norm` — **크롭용 대략 좌표.** 프롬프트에서 "대략이어도 된다, 값을
  정확히 읽는 것이 훨씬 중요하다, 넉넉하게 잡아라" 를 명시한다. 좌표 정확도를
  압박하면 그 용량이 전사 정확도에서 빠져나간다.
...
**VLM 의 좌표는 신뢰하지 않고, VLM 의 텍스트를 신뢰한다.**

크롭을 넉넉히(35%, 최소 12px) 떠서 2배 업샘플 → 배치 OCR → 크롭 안에서 VLM 이
읽은 값과 같은 텍스트 줄을 찾는다. 찾으면 **그 OCR 박스의 좌표**가 최종 좌표다.

탐색은 좁은 답부터 간다.

| 순서 | 방식 | `char_span` |
|---|---|---|
| ① | 박스 1개 안에서 완전일치 | 있음 (부분 마스킹 가능) |
| ② | 인접 박스 이어붙여 완전일치 (`"010-1234"` + `"5678"`) | 없음 |
| ③ | 유사 매칭 — `NAME`/`ADDRESS`/`ORG`/`TITLE` **만** | 있음 |

좁은 답을 먼저 찾는 것이 중요하다. 이어붙이기를 먼저 시도하면 값 하나에 옆 칸이
딸려 들어와 박스가 셀 두 개를 덮는다.

유사 매칭을 번호류에 **쓰지 않는** 이유: 숫자는 한 글자 다르면 "오독된 같은
번호" 가 아니라 **그냥 다른 번호**다. 유사도로 이어붙이면 옆 칸의 다른 사람
주민번호에 붙는다.

1차에서 못 찾으면 크롭을 넓혀 **한 번만** 재시도한다. 그래서 OCR 배치는 두 번
돈다 (전부 → 실패분만).

### 교차검증이 공짜로 나온다

두 엔진이 독립적으로 같은 값을 읽었다면 그 값은 거의 확실하다. 못 찾은 경우는
세 가지로 나뉘고, 각각 다르게 처리한다.

| 상황 | `source` | `needs_review` | `reason` |
|---|---|---|---|
| OCR 이 값을 찾음 | `ocr_refined` | 불일치일 때만 | 일치 방식 |
| OCR 이 아무것도 못 읽음 | `vlm_coarse` | `true` | 손글씨/도장 추정 |
| OCR 은 읽었는데 값이 다름 | `vlm_coarse` | `true` | **양쪽 원문 기록** |
| 읽을 글자가 애초에 없음 (서명) | `vlm_coarse` | `false` | 서명·인영 |

세 번째가 진단의 핵심이다.

```
"크롭 OCR 이 VLM 의 값을 찾지 못했다. VLM '901112-2846261' vs OCR '9O1112-284626'"
```
...
→ **720건 통과**면 체크섬·회피 표기 정규화·타일 좌표 환산·크롭 매칭·검증·
...
```
② VLM 탐지  타일 3개 (겹침 8%)  image_max_side=2000  타일 세로 약 826px — 축소 없음
③ 좌표 확정  크롭 패딩 35% (최소 12px)  업샘플 x2.0  유사매칭 0.65
④ 검증      체크섬 교정=ON  항목명 제외=ON
```
...
locate:            # ③ 좌표 확정
  ...
  similarity: 0.65 # 유사 매칭 임계값 (이름/주소/기관/직위만)

verify:            # ④ 검증
  retype_on_checksum: true   # 체크섬으로 증명되면 type 교정
  drop_field_labels: true    # "성명" 같은 헤더 셀 제외
...
> **`max_tokens` 를 512 로 내리지 말 것.** 한 항목이 `text`/`field`/`bbox_norm`
...
| `--similarity` | 유사 매칭 임계값 (기본 0.65). `1.0` 이면 완전일치만 |
...
로직·배선은 720건의 테스트로 검증됐다. PDF 렌더링은 실제 pypdfium2 로 검증했다.
