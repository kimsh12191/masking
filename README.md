...
부수 효과가 크다. 페이지를 통째로 보내면 `image_max_side` 에서 0.64배로 축소돼
주민등록번호 숫자가 10px 대로 떨어진다. 3조각이면 조각 긴 변이 1760 이라
**축소가 아예 일어나지 않는다.**

**모든 크기를 패치 격자(32px)에 맞춘다.** 비전 인코더는 이미지를
`image_factor`(= 패치 16 × merge 2 = 32) 의 배수로 리사이즈한다. 배수가 아니면
서버가 조각을 다시 리샘플하고 10px 급 한글이 보간으로 뭉개진다 — 곧 미탐이다.
그래서 `target_long_side` 2464(=32×77), 전처리가 짧은 변에 여백을 붙여 정렬,
조각 경계를 32 배수로 스냅, `image_max_side` 1984(=32×62), `min_pad_px` 64(=2×32).

```
A4 1748×2480 → 전처리 1760×2464 → 조각 1760×896 / 992 / 896 → 모델 그대로
                (둘 다 ÷32)        (전부 ÷32)              리샘플 0회
```

경계 스냅 때문에 조각이 균등하지 않다(896/992/896). 그건 문제가 아니고,
리샘플이 사라지는 것이 훨씬 크다. `--print-config` 의 `격자 정합` 줄이 이
값들을 검산한다 — 어긋나도 실행은 되고 결과만 나빠지므로 검산이 필요하다.
