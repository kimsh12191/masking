...
→ **568건 통과 / 21건 skip**(Pillow·numpy 미설치 시 skip 이 늘어난다)이면
체크섬·정규식·읽기순서 정렬·병합 검증·값 전파·프롬프트·설정 로딩·라벨 스키마·
PDF 페이지 분해·파이프라인 배선이 모두 정상이다.
...
| 단계 | 방식 | `needs_review` |
|---|---|---|
| ① 완전일치 | 구분자·공백 무시 후 포함 검사 | `false` |
| ② 혼동문자 접기 | `O`↔`0`, `l`↔`1`, `S`↔`5` … 접은 뒤 완전일치. **숫자 비율이 높은 값에만** | `true` |
| ③ 유사도 | 한글 오독 등 혼동표로 못 잡는 것 | `true` |

②가 별도로 필요한 이유: 11자리 번호에 오독이 **한 글자만** 있어도
`SequenceMatcher` 비율이 0.909 로 떨어진다. 유사도 임계값을 그 위에 두면
아무것도 안 걸리고, 아래로 내리면 전혀 다른 번호가 걸린다. 번호류는 유사도가
아니라 혼동쌍을 명시적으로 접어야 정확하다.
...
- `ORG`/`TITLE` 은 **기본 제외**. `하나은행`, `과장` 같은 값이 서식 전체에
  깔려 있어 전파하면 문서가 통째로 마스킹된다 (필요하면 `propagate_labels` 로 opt-in)
...
propagate:
  ...
  fold_confusables: true                 # O↔0, l↔1 오독 흡수 (번호류만)
