...
| 학습 샘플 | |
|---|---|
| **입력** | 타일 이미지 **1760×960 고정** (추론에서 모델이 받는 것과 동일) |
| **정답** | `{"findings":[{"text":"강동혁","bbox_2d":[318,266,398,317]}, ...]}` |
| 손실 | 정답 토큰만. 프롬프트는 마스킹 |

**모든 샘플의 입력 크기가 같습니다.** 페이지가 고정 캔버스(1760×2464)이고,
타일 3개가 전부 1760×960 입니다.

`--aug-scales 1.5,2.0` 은 같은 종횡비의 더 작은 영역을 잘라 타일 크기로
확대합니다 — **입력 크기는 그대로, 글자만 1.5~2배로 보입니다.** 캔버스를
고정해도 문서마다 폰트 크기가 다르므로, 한 가지 스케일만 학습하면 나머지에서
좌표가 흔들립니다.
