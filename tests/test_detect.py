...
class FakeClient:
    ...
    def complete_json(self, system: str, user: str, schema: dict, image: Any = None):
        n = len(self.calls)
        self.calls.append({"system": system, "user": user, "image": image})
...
class TestDetect:
    def test_calls_once_per_tile(self) -> None:
        ...
        detect(blank_page(), client, DetectConfig(tiles=3))
    ...
    def test_tile_local_coords_become_page_coords(self) -> None:
        ...
        findings, _ = detect(blank_page(), client, DetectConfig(tiles=2, overlap=0.0))
    ...
    def test_records_tile_number(self) -> None:
        ...
        findings, _ = detect(blank_page(), client, DetectConfig(tiles=2))
        assert findings[0].tile == 1

    def test_sends_an_image_every_call(self) -> None:
        client = FakeClient([{"findings": []}] * 2)
        detect(blank_page(), client, DetectConfig(tiles=2))
    ...
    def test_single_tile_sends_the_untouched_page(self) -> None:
        ...
        detect(page, client, DetectConfig(tiles=1))
    ...
    def test_one_failed_tile_does_not_lose_the_others(self) -> None:
        ...
        findings, metas = detect(blank_page(), client, DetectConfig(tiles=3), warnings)
    ...
    def test_salvaged_response_raises_a_warning(self) -> None:
        ...
        detect(blank_page(), client, DetectConfig(tiles=1), warnings)
    ...
    def test_item_without_bbox_is_dropped_with_a_warning(self) -> None:
        ...
        findings, _ = detect(blank_page(), client, DetectConfig(tiles=1), warnings)
        assert findings == []
    ...
    def test_item_without_type_is_skipped(self) -> None:
        ...
        findings, _ = detect(blank_page(), client, DetectConfig(tiles=1))
        assert findings == []
    ...
    def test_dedup_can_be_disabled(self) -> None:
        ...
        findings, _ = detect(
            blank_page(), client, DetectConfig(tiles=2, overlap=0.5, dedup=False)
        )
        assert len(findings) == 2

    def test_hint_reaches_the_user_message(self) -> None:
        client = FakeClient([{"findings": []}])
        detect(blank_page(), client, DetectConfig(tiles=1, hint="가족관계증명서다"))
